...
console.log(`\r\nfile.js: mongoUrl: ${mongoUrl} `);
console.log(`file.js: dbname: ${dataBaseName} `);
...
conn.once('open', () => {
    //Init stream
    cur= Grid(conn.db, mongoose.mongo);
    cur.collection('uploads');
...
router.get('/files', (req,res) => {
    ...
    cur.files.find().toArray((err,files) => {
...
router.get('/file/:filename', (req,res) => {
    ...
    cur.files.findOne({ filename: req.params.filename},(err,file) => {
        console.log(`File ${file}`);
        // Check if file
...
router.get('/image/:filename', (req,res) => {
    cur.files.findOne({ filename: req.params.filename},(err,file) => {
        ...
        if(!file)  {
            ...
            const readstream = cur.createReadStream(file.filename);
            readstream.pipe(res);
        } else {
...
router.delete('/dfiles/:id', (req, res) => {
    //console.log(` _id: ${_id} Req: ${req.params.id}`);

    cur.remove({_id: req.params.id, root: 'uploads'}, (err, gridStore) => {
        if(err) {
    ...
    cur.files.find().toArray((err,files) => {
        ...
        if(!files || files.length === 0) {
            ...
            files.forEach(element => {
                ...
                console.log(`Index: ${index}  _id: ${element._id} Filename: ${element.filename}`);

                if(index++<15)  {
                cur.remove({_id: element._id, root: 'uploads'}, (err, gridStore) => {
                    if(err) {
                        console.log(`Error deleting file ${element.filename}`);
                        //return res.status(404).json({ err: err });
                    }
