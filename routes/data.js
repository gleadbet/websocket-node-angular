...
// Note: Dataset to keep track of most recent data?  - dont' have -- only way now -- look at the most recent set
//  Give you most recent dataset, use below -- need to find tag in mongo to get fully qualified name, 
//  based on the description Inlet Temp -- CAM-TS-01:MultiCoat.J1_TT_0
//  db.dataset.find({name:"CAM-TS-01:MultiCoat.J1_TT_01"}).sort({"startTime":-1}).limit(1).pretty()
// [11:49 AM] Eric Holterman  db.data.find({_id: ObjectId("63eec38101edda074018d649")}).pretty()

        var pipeline = [
            {$match: {name: {'$in': explist}}},    // match datsets (long .. no time param) - check last 24hrs/ no .. then check over time to list pull all tags in explist?  - should it be updated? Array of tags 
            {$sort: {startTime: -1, _id: -1}},     // Sort by time/id ?
            {$group: {_id: {distinctName: "$name"}, lastdataset: {$first: {lastData: "$lastData"}}}},  // group by tag
            {$lookup: {from: "data", localField: "lastdataset.lastData", foreignField: "_id", as: "dataDoc"}}, // lookup by data
            {$unwind: "$dataDoc"},
            {$project: {_id: 0, Tagname: "$_id.distinctName", "LatestRecord": {$arrayElemAt: ["$dataDoc.values", -1]}}},  // -1 get last
 ...
 * @apiSuccess {Object} value Single data value object after the requested
 */
// Comments out autorization for test
router.put("/timeseriesdata/:id", async (req, res) => {
